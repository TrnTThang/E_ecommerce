	...
	<div class="container">
		<div class="row">
			...
			if(isset($_POST['form_subscribe']))
			{
				...
				// 			$verify_link = 'https://thekymobile.com/verify.php?email='.$to.'&token='.$token;
							// Getting the url of the verification link
							$verification_url = 'https://thekymobile.com/verify.php?email='.$to.'&key='.$key;
...
This link will be active only for 24 hours.
			...
			?>
				<h1 style="color:white">Đăng kí nhận tin</h1>
			         	<span class="input-group-btn">
			       <a href="mail.php" > <button class="btn btn-theme" type="submit" name="form_subscribe">Mailchimp</button> </a>
			         	</span>
			        </div>
				</div>
				</form>
	...
	$(document).ready(function () {
        ...
        $('#advFieldsStatus').on('change',function() {
            ...
            if ( advFieldsStatus == '' ) {
				...
				$('#stripe_form').show();
				$('#bank_form').hide();
            } else if ( advFieldsStatus == 'Tien-mat' ) {
            	$('#paypal_form').hide();
				$('#stripe_form').hide();
				$('#bank_form').show();
            }
