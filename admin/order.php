...
Card Year: '.$row['card_year'].'<br>
        		';
        	elseif($row['payment_method'] == 'Thanh toán khi nhận hàng'):
				$payment_details = '
...
Thời gian đặt hàng: '.$row['payment_date'].'<br>
Chi tiết đơn hàng: <br>'.$payment_details.'<br>
Tổng giá trị đơn hàng: '.number_format($row['paid_amount']).' ₫<br>
Trạng thái thanh toán: '.$row['payment_status'].'<br>
Trạng thái giao hàng: '.$row['shipping_status'].'<br>
Mã đơn hàng: '.$row['payment_id'].'<br>
            ';
        }
...
Giá sản phẩm: '.number_format($row['unit_price']).' ₫<br>
            ';
        }
          ...
          <table id="example1" class="table table-bordered table-hover table-striped">
            ...
            <tbody>
            	...
            	foreach ($result as $row) {
            		$i++;
            		?>
					<tr class="<?php if($row['payment_status']=='Chua-hoan-thanh'){echo 'bg-r';}else{echo 'bg-g';} ?>">
	                    <td><?php echo $i; ?></td>
	                    <td>
                        ...
                        <td>
                           ...
                           foreach ($result1 as $row1) {
                                ...
                                echo ', <b>Giá:</b> '.number_format($row1['unit_price']).' ₫)';
                                echo '<br><br>';
                           }
                           ?>
                        </td>
                        <td>
                        	...
                        	<?php elseif($row['payment_method'] == 'Tien-mat'): ?>
                        		<b>Phương thức thanh toán</b> <?php echo '<span style="color:red;"><b>'.$row['payment_method'].'</b></span>'; ?><br>
                        		<b>Mã đơn hàng:</b> <?php echo $row['payment_id']; ?><br>
                        ...
                        <td>
                            ...
                            <br><br>
                            <?php
                                if($row['payment_status']=='Chua-hoan-thanh'){
                                    ?>
                                    <a href="order-change-status.php?id=<?php echo $row['id']; ?>&task=Hoan-thanh" class="btn btn-success btn-xs" style="width:100%;margin-bottom:4px;">Đánh dấu hoàn tất</a>
                                    <?php
                                }
                            ?>
                        </td>
                        <td>
                            ...
                            <br><br>
                            <?php
                            if($row['payment_status']=='Hoan-thanh') {
                                if($row['shipping_status']=='Chua-hoan-thanh'){
                                    ?>
                                    <a href="shipping-change-status.php?id=<?php echo $row['id']; ?>&task=Hoan-thanh" class="btn btn-warning btn-xs" style="width:100%;margin-bottom:4px;">Đánh dấu hoàn tất</a>
                                    <?php
                                }
                            }
                            ?>
